...
- Model management
    ...
    def load_data_features(self):
        ...
        try:
            ...
            self.status_var.set(f"Loaded {len(features)} features from {self.data_file}")
    ...
    def refresh_mpeg_files_from_dir(self, plots_dir):
        ...
        try:
            ...
            # Update listbox with proper widget existence checks
            if hasattr(self, 'mpeg_files_listbox') and self.mpeg_files_listbox is not None:
                try:
                    # Check if the widget is still valid (not destroyed)
                    if self.mpeg_files_listbox.winfo_exists():
                        self.mpeg_files_listbox.delete(0, tk.END)
                        if animation_files:
                        ...
                        print("MPEG files listbox has been destroyed")
                except tk.TclError:
                    print("MPEG files listbox has been destroyed")
                except Exception as e:
                    print(f"Error updating MPEG files listbox: {e}")
            else:
                print("MPEG files listbox not found")
