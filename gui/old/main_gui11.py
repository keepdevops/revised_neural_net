...
- Model management
    ...
    def show_3d_controls(self):
        ...
        try:
            ...
            controls_window.geometry("450x700")
            ...
            # Title
            title_label = ttk.Label(main_frame, text="🎮 3D Visualization Controls", 
                                   font=("Arial", 14, "bold"))
            title_label.grid(row=0, column=0, pady=(0, 20))
            
            # Plot Type Section
            plot_frame = ttk.LabelFrame(main_frame, text="Plot Type", padding="10")
            plot_frame.grid(row=1, column=0, sticky="ew", pady=(0, 10))
            plot_frame.grid_columnconfigure(0, weight=1)
            
            plot_type_label = ttk.Label(plot_frame, text="Visualization Type:")
            plot_type_label.grid(row=0, column=0, sticky="w", pady=(0, 5))
            
            plot_type_combo = ttk.Combobox(plot_frame, textvariable=self.vis_type_var, 
                                          values=["Wireframe", "Surface", "Contour", "Scatter"],
                                          state="readonly")
            plot_type_combo.grid(row=1, column=0, sticky="ew", pady=(0, 10))
            plot_type_combo.bind('<<ComboboxSelected>>', self.update_3d_plot_type)
            ...
            color_label = ttk.Label(color_frame, text="Color Map:")
            ...
            color_combo = ttk.Combobox(color_frame, textvariable=self.color_var,
                                      values=["viridis", "plasma", "inferno", "magma", "cividis", 
                                             "coolwarm", "RdBu", "Spectral", "rainbow", "jet"],
                                      state="readonly")
            color_combo.grid(row=1, column=0, sticky="ew", pady=(0, 10))
            color_combo.bind('<<ComboboxSelected>>', self.on_color_map_change)
            
            # Range Controls Section
            range_frame = ttk.LabelFrame(main_frame, text="Weight Ranges", padding="10")
            range_frame.grid(row=3, column=0, sticky="ew", pady=(0, 10))
            range_frame.grid_columnconfigure(1, weight=1)
            range_frame.grid_columnconfigure(3, weight=1)
            
            # W1 Range
            w1_label = ttk.Label(range_frame, text="W1 Range:")
            w1_label.grid(row=0, column=0, sticky="w", pady=(0, 5))
            
            w1_min_entry = ttk.Entry(range_frame, textvariable=self.w1_range_min_var, width=8)
            w1_min_entry.grid(row=1, column=0, padx=(0, 5), pady=(0, 5))
            w1_min_entry.bind('<KeyRelease>', self.on_w1_range_change)
            
            w1_to_label = ttk.Label(range_frame, text="to")
            w1_to_label.grid(row=1, column=1, padx=5, pady=(0, 5))
            
            w1_max_entry = ttk.Entry(range_frame, textvariable=self.w1_range_max_var, width=8)
            w1_max_entry.grid(row=1, column=2, padx=(5, 0), pady=(0, 5))
            w1_max_entry.bind('<KeyRelease>', self.on_w1_range_change)
            
            # W2 Range
            w2_label = ttk.Label(range_frame, text="W2 Range:")
            w2_label.grid(row=2, column=0, sticky="w", pady=(10, 5))
            
            w2_min_entry = ttk.Entry(range_frame, textvariable=self.w2_range_min_var, width=8)
            w2_min_entry.grid(row=3, column=0, padx=(0, 5), pady=(0, 5))
            w2_min_entry.bind('<KeyRelease>', self.on_w2_range_change)
            
            w2_to_label = ttk.Label(range_frame, text="to")
            w2_to_label.grid(row=3, column=1, padx=5, pady=(0, 5))
            
            w2_max_entry = ttk.Entry(range_frame, textvariable=self.w2_range_max_var, width=8)
            w2_max_entry.grid(row=3, column=2, padx=(5, 0), pady=(0, 5))
            w2_max_entry.bind('<KeyRelease>', self.on_w2_range_change)
            
            # Points Control
            points_label = ttk.Label(range_frame, text="Number of Points:")
            points_label.grid(row=4, column=0, sticky="w", pady=(10, 5))
            
            points_entry = ttk.Entry(range_frame, textvariable=self.n_points_var, width=8)
            points_entry.grid(row=5, column=0, padx=(0, 5), pady=(0, 5))
            points_entry.bind('<KeyRelease>', self.on_n_points_change)
            
            # View Controls Section
            view_frame = ttk.LabelFrame(main_frame, text="3D View Controls", padding="10")
            view_frame.grid(row=4, column=0, sticky="ew", pady=(0, 10))
            view_frame.grid_columnconfigure(0, weight=1)
            view_frame.grid_columnconfigure(1, weight=1)
            
            # View buttons in a grid
            reset_btn = ttk.Button(view_frame, text="🔄 Reset View", command=self.reset_3d_view)
            reset_btn.grid(row=0, column=0, padx=(0, 5), pady=5, sticky="ew")
            
            top_btn = ttk.Button(view_frame, text="⬆️ Top View", command=self.set_top_view)
            top_btn.grid(row=0, column=1, padx=(5, 0), pady=5, sticky="ew")
            
            side_btn = ttk.Button(view_frame, text="➡️ Side View", command=self.set_side_view)
            side_btn.grid(row=1, column=0, padx=(0, 5), pady=5, sticky="ew")
            
            iso_btn = ttk.Button(view_frame, text="📐 Isometric", command=self.set_isometric_view)
            iso_btn.grid(row=1, column=1, padx=(5, 0), pady=5, sticky="ew")
            
            # Animation Controls Section
            anim_frame = ttk.LabelFrame(main_frame, text="Animation Controls", padding="10")
            anim_frame.grid(row=5, column=0, sticky="ew", pady=(0, 10))
            anim_frame.grid_columnconfigure(0, weight=1)
            
            # MPEG Generation
            mpeg_btn = ttk.Button(anim_frame, text="🎬 Generate MPEG Animation", 
                                 command=self.generate_mpeg_animation)
            mpeg_btn.grid(row=0, column=0, pady=5, sticky="ew")
            
            # Progress bar for MPEG generation
            self.mpeg_progress = ttk.Progressbar(anim_frame, mode='indeterminate')
            self.mpeg_progress.grid(row=1, column=0, pady=5, sticky="ew")
            
            # Status label
            self.floating_status_label = ttk.Label(anim_frame, text="Ready", 
                                                  foreground="green")
            self.floating_status_label.grid(row=2, column=0, pady=5)
            ...
            refresh_btn = ttk.Button(actions_frame, text="🔄 Refresh 3D Plot", 
                                   command=self.update_3d_plot_type)
            refresh_btn.grid(row=0, column=0, pady=5, sticky="ew")
            
            # Close button
            close_btn = ttk.Button(main_frame, text="❌ Close", 
                                 command=controls_window.destroy)
            close_btn.grid(row=7, column=0, pady=(10, 0), sticky="ew")
            
            # Set focus to the window
            controls_window.focus_set()
