            ...
            def run_mpeg_generation():
                try:
                    ...
                    if hasattr(self, 'view_elev_var') and self.view_elev_var.get():
                        cmd.extend(["--view_elev", self.view_elev_var.get()])
                    if hasattr(self, 'view_azim_var') and self.view_azim_var.get():
                        cmd.extend(["--view_azim", self.view_azim_var.get()])
                    if hasattr(self, 'res_var') and self.res_var.get():
                        # Parse resolution (e.g., "1920x1080" -> ["1920", "1080"])
                        res_parts = self.res_var.get().split('x')
                        if len(res_parts) == 2:
                            cmd.extend(["--output_resolution", res_parts[0], res_parts[1]])
                    
                    # Add 3D plot parameters if available
                    if hasattr(self, 'color_var') and self.color_var.get():
                        cmd.extend(["--color", self.color_var.get()])
                    if hasattr(self, 'w1_range_min_var') and hasattr(self, 'w1_range_max_var'):
                        if self.w1_range_min_var.get() and self.w1_range_max_var.get():
                            cmd.extend(["--w1_range", self.w1_range_min_var.get(), self.w1_range_max_var.get()])
                    if hasattr(self, 'w2_range_min_var') and hasattr(self, 'w2_range_max_var'):
                        if self.w2_range_min_var.get() and self.w2_range_max_var.get():
                            cmd.extend(["--w2_range", self.w2_range_min_var.get(), self.w2_range_max_var.get()])
                    if hasattr(self, 'n_points_var') and self.n_points_var.get():
                        cmd.extend(["--n_points", self.n_points_var.get()])
