...
- Model management
    ...
    def show_3d_controls(self):
        ...
        try:
            ...
            # Animation buttons
            ttk.Button(anim_frame, text="Generate MPEG Animation", command=self.generate_mpeg_animation).grid(row=0, column=0, sticky="ew", pady=(0, 5))
            ttk.Button(anim_frame, text="Generate GIF Animation", command=self.generate_gif_animation).grid(row=1, column=0, sticky="ew", pady=(0, 5))
            ttk.Button(anim_frame, text="Browse MPEG Files", command=self.browse_mpeg_files).grid(row=2, column=0, sticky="ew", pady=(0, 5))
            ttk.Button(anim_frame, text="Refresh MPEG Files", command=self.refresh_mpeg_files).grid(row=3, column=0, sticky="ew", pady=(0, 5))
            
            # MPEG Files Listbox
            ttk.Label(anim_frame, text="Available Animation Files:").grid(row=4, column=0, sticky="w", pady=(10, 5))
            
            # Create frame for listbox and scrollbar
            listbox_frame = ttk.Frame(anim_frame)
            listbox_frame.grid(row=5, column=0, sticky="ew", pady=(0, 5))
            listbox_frame.grid_columnconfigure(0, weight=1)
            listbox_frame.grid_rowconfigure(0, weight=1)
            
            # Create listbox for MPEG files
            mpeg_listbox = tk.Listbox(listbox_frame, height=4, selectmode=tk.SINGLE)
            mpeg_listbox.grid(row=0, column=0, sticky="ew", padx=(0, 5))
            mpeg_listbox.bind("<<ListboxSelect>>", self.on_mpeg_file_select)
            
            # Create scrollbar for listbox
            mpeg_scrollbar = ttk.Scrollbar(listbox_frame, orient="vertical", command=mpeg_listbox.yview)
            mpeg_scrollbar.grid(row=0, column=1, sticky="ns")
            mpeg_listbox.configure(yscrollcommand=mpeg_scrollbar.set)
            
            # Store reference to listbox
            self.mpeg_files_listbox = mpeg_listbox
            
            # MPEG file action buttons
            mpeg_buttons_frame = ttk.Frame(anim_frame)
            mpeg_buttons_frame.grid(row=6, column=0, sticky="ew", pady=(5, 0))
            mpeg_buttons_frame.grid_columnconfigure(0, weight=1)
            mpeg_buttons_frame.grid_columnconfigure(1, weight=1)
            
            ttk.Button(mpeg_buttons_frame, text="Open Selected", command=self.open_selected_mpeg).grid(row=0, column=0, sticky="ew", padx=(0, 5))
            ttk.Button(mpeg_buttons_frame, text="Clear Selection", command=lambda: mpeg_listbox.selection_clear(0, tk.END)).grid(row=0, column=1, sticky="ew", padx=(5, 0))
            
            # Initialize MPEG files list
            self.refresh_mpeg_files()
    ...
    def generate_gif_animation(self):
        ...
        try:
            ...
            if hasattr(self, 'mpeg_progress'):
                ...
                print("✅ GIF progress bar started")
            ...
            def run_gif_generation():
                try:
                    ...
                    # Build command
                    cmd = [python_exe, script_path, "--model_dir", self.selected_model_path, "--save_gif"]
                    ...
                    if result.returncode == 0:
                        ...
                        self.root.after(0, self._gif_generation_completed_success)
                    else:
                        error_msg = f"GIF generation failed: {result.stderr}"
                        print(f"❌ {error_msg}")
                        self.root.after(0, lambda: self._gif_generation_completed_error(error_msg))
                        
                except Exception as e:
                    error_msg = f"Error generating GIF: {str(e)}"
                    print(f"❌ {error_msg}")
                    self.root.after(0, lambda: self._gif_generation_completed_error(error_msg))
            
            # Start the thread
            import threading
            thread = threading.Thread(target=run_gif_generation, daemon=True)
            thread.start()
            
            self.status_var.set("Generating GIF animation...")
            
        except Exception as e:
            print(f"❌ Error starting GIF generation: {e}")
            messagebox.showerror("Error", f"Error starting GIF generation: {str(e)}")

    def _gif_generation_completed_success(self):
        """Handle successful GIF generation."""
        try:
            if hasattr(self, 'mpeg_progress'):
                self.mpeg_progress.stop()
                print("✅ GIF progress bar stopped")
            
            self.status_var.set("GIF animation generated successfully")
            messagebox.showinfo("Success", "GIF animation generated successfully!")
            
            # Refresh MPEG files to show the new GIF
            self.refresh_mpeg_files()
            
        except Exception as e:
            print(f"❌ Error handling GIF generation success: {e}")

    def _gif_generation_completed_error(self, error_msg):
        """Handle GIF generation error."""
        try:
            if hasattr(self, 'mpeg_progress'):
                self.mpeg_progress.stop()
                print("✅ GIF progress bar stopped")
            
            self.status_var.set(f"GIF generation failed: {error_msg}")
            messagebox.showerror("Error", f"GIF generation failed: {error_msg}")
            
        except Exception as e:
            print(f"❌ Error handling GIF generation error: {e}")
