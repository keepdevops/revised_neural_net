...
class StockPredictionGUI:
    def __init__(self, root):
        ...
        # In __init__ or initialize_3d_variables, add:
        # self.is_3d_viewer_floating = False
        # self.floating_3d_viewer = None
        # self.plot_frame = None

        # In the control panel setup (where plot_frame is created), after plot_frame is created:
        # self.app.plot_frame = plot_frame
        # Add a button:
        # popout_btn = ttk.Button(plot_frame, text="Pop Out 3D Viewer", command=self.app.popout_3d_viewer)
        # popout_btn.grid(row=2, column=0, sticky="ew", pady=(5,0))
        # (Make sure row=2 is not used by anything else)
...
- Model management
    ...
    def make_prediction(self):
        ...
        try:
            ...
            # Load the model's feature information
            feature_info_file = os.path.join(self.selected_model_path, 'feature_info.json')
            if os.path.exists(feature_info_file):
                try:
                    with open(feature_info_file, 'r') as f:
                        feature_info = json.load(f)
                    x_features = feature_info.get('x_features', [])
                    y_feature = feature_info.get('y_feature', 'close')
                    print(f"📋 Using model's trained features: {x_features}")
                    print(f"🎯 Using model's target feature: {y_feature}")
                except Exception as e:
                    print(f"❌ Error loading feature info: {e}")
                    messagebox.showerror("Error", f"Error loading model feature information: {str(e)}")
                    return
            else:
                print("⚠️ No feature_info.json found, using default features")
                # Fallback to default features if no feature info file
                x_features = ['open', 'high', 'low', 'vol']
                y_feature = 'close'
            
            # Validate that the required features exist in the data file
            try:
                df_sample = pd.read_csv(self.data_file, nrows=1)
                missing_features = [f for f in x_features if f not in df_sample.columns]
                if missing_features:
                    error_msg = f"The following required features are missing from your data file:\n{missing_features}\n\nAvailable features: {list(df_sample.columns)}"
                    messagebox.showerror("Missing Features", error_msg)
                    return
                
                if y_feature not in df_sample.columns:
                    error_msg = f"The target feature '{y_feature}' is missing from your data file.\n\nAvailable features: {list(df_sample.columns)}"
                    messagebox.showerror("Missing Target Feature", error_msg)
                    return
                    
            except Exception as e:
                print(f"❌ Error validating data file: {e}")
                messagebox.showerror("Error", f"Error validating data file: {str(e)}")
                return
...
*Stock Prediction Neural Network GUI v1.0*
    ...
    def popout_3d_viewer(self):
        """Pop out the 3D viewer into a floating window."""
        if hasattr(self, 'floating_3d_viewer') and self.floating_3d_viewer is not None:
            self.floating_3d_viewer.lift()
            return
        import tkinter as tk
        self.floating_3d_viewer = tk.Toplevel(self.root)
        self.floating_3d_viewer.title("3D Viewer")
        self.floating_3d_viewer.protocol("WM_DELETE_WINDOW", self.dock_3d_viewer)
        # Move canvas and toolbar to the floating window
        if hasattr(self, 'gd3d_canvas') and self.gd3d_canvas is not None:
            self.gd3d_canvas.get_tk_widget().master = self.floating_3d_viewer
            self.gd3d_canvas.get_tk_widget().pack(fill="both", expand=True)
        if hasattr(self, 'gd3d_toolbar') and self.gd3d_toolbar is not None:
            self.gd3d_toolbar.master = self.floating_3d_viewer
            self.gd3d_toolbar.pack(fill="x")
        # Add dock button
        dock_btn = tk.Button(self.floating_3d_viewer, text="Dock Viewer", command=self.dock_3d_viewer)
        dock_btn.pack(side="bottom", pady=5)
        self.is_3d_viewer_floating = True
        self.status_var.set("3D viewer popped out")

    def dock_3d_viewer(self):
        """Dock the 3D viewer back into the Plot Controls tab."""
        if not hasattr(self, 'floating_3d_viewer') or self.floating_3d_viewer is None:
            return
        # Find the original plot_frame in the Plot Controls tab
        if hasattr(self, 'gd3d_canvas') and self.gd3d_canvas is not None and hasattr(self, 'plot_frame'):
            self.gd3d_canvas.get_tk_widget().master = self.plot_frame
            self.gd3d_canvas.get_tk_widget().grid(row=0, column=0, sticky="nsew")
        if hasattr(self, 'gd3d_toolbar') and self.gd3d_toolbar is not None and hasattr(self, 'plot_frame'):
            self.gd3d_toolbar.master = self.plot_frame
            self.gd3d_toolbar.grid(row=1, column=0, sticky="ew")
        self.floating_3d_viewer.destroy()
        self.floating_3d_viewer = None
        self.is_3d_viewer_floating = False
        self.status_var.set("3D viewer docked")
