...
class Plot3DWindow:
    def __init__(self, parent, fig, toolbar=None, on_close=None):
        ...
        if toolbar is not None:
            toolbar.master = self.window
            toolbar.pack(fill="x")

        # Dock button (optional)
        dock_btn = tk.Button(self.window, text="Dock Viewer", command=self._on_close)
...
*Stock Prediction Neural Network GUI v1.0*
    ...
    def popout_3d_viewer(self):
        ...
        if hasattr(self, 'floating_3d_viewer') and self.floating_3d_viewer is not None:
            self.floating_3d_viewer.window.lift()
            return
        ...
        self.floating_3d_viewer = Plot3DWindow(
            parent=self.root,
            fig=self.gd3d_fig,
            toolbar=self.gd3d_toolbar if hasattr(self, 'gd3d_toolbar') else None,
            on_close=on_close
        )
